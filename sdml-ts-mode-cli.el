;;; sdml-ts-mode-cli.el --- CLI integration -*- lexical-binding: t; -*-
...
(defcustom sdml-ts-mode-cli-name "sdml"
...
(defcustom sdml-ts-mode-cli-no-color nil
...
(defconst sdml-ts-mode-cli-default-output-buffer-name "*SDML Command-Line Output*")

(defconst sdml-ts-mode-cli-default-error-buffer-name "*SDML Command-Line Errors*")

(defun sdml-ts-mode-cli-make-arg (name value)
  "Make an argument string from NAME and VALUE."
  (format "--%s %s" name value))

(defun sdml-ts-mode-cli-make-command (command &rest args)
...
(defun sdml-ts-mode-cli-run-command (command &optional output-buffer-name error-buffer-name refresh-fn)
